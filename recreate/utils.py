...
def buildjoint(data, n=1):
    ...
    if joint_type == "ball":
        for i in range(3):
            ...
            s = f'<joint type="hinge" name="{name}_{i}" axis="{" ".join(axis)}" pos="{pos}" range="{r}" limited="true" />'
            xml += tabs(n) + s
        ...
        if "LimLow0" in data and "LimHigh0" in data and data["LimLow0"] < data["LimHigh0"]:
            xml += f' range="{data["LimLow0"]} {data["LimHigh0"]}" limited="true"'
...
def mjcf(data):
    ...
    <worldbody>
        <geom condim="3" friction="1 .1 .1" material="MatPlane" name="floor" pos="0 0 -0.886" rgba="0.8 0.9 0.8 1" size="20 20 0.125" type="plane"/>'''
    ...
    for j, joint in joint_map.items():
        ...
        if joint["Type"] == "spherical":
            for i in range(3):
                mjcf_str += tabs(2) + f'<motor joint="{name}_{i}" name="{name}_{i}" ctrllimited="true" ctrlrange="-1 1" gear="{joint["TorqueLim"] / 1}" />'
        else:
            mjcf_str += tabs(2) + f'<motor joint="{name}" name="{name}" ctrllimited="true" ctrlrange="-1 1" gear="{joint["TorqueLim"] / 1}" />'
...
def quaternion_to_euler(quat, order="XYZ"):
    ...
    rotation = R.from_quat([quat[1], quat[3], quat[2], quat[0]])  # SciPy uses [x, y, z, w]
...
def translate_joints(data):
    ...
    # chest_rot = [chest_rot[0], -chest_rot[1], chest_rot[2]]
    neck_rot = quaternion_to_euler(data[12:16])
    # neck_rot = [neck_rot[0], -neck_rot[1], neck_rot[2]]
    ...
    # right_shoulder_rot = [right_shoulder_rot[0], -right_shoulder_rot[1], right_shoulder_rot[2]]
    ...
    # left_ankle_rot = [left_ankle_rot[0], -left_ankle_rot[1], left_ankle_rot[2]]
    
    left_shoulder_rot = quaternion_to_euler(data[39:43])
    # left_shoulder_rot = [left_shoulder_rot[0], -left_shoulder_rot[1], left_shoulder_rot[2]]
...
def motion_to_posvel(frames, model, data):
    ...
    pos, rots, linvels, angvels, coms, ends = [], [], [], [], [], []
    nqp = []
    dur = 0
    for i, frame in enumerate(frames):
        fpos = np.array(translate_joints(frame))
        ...
        nqp.append(data.qpos)
        # linvels.append(data.cvel[1:, :3])
        # angvels.append(data.cvel[1:, 3:])
        vel = np.zeros_like(pos[0])
        if i > 0:
            vel = (pos[i] - pos[i - 1]) / frames[i - 1][0]
        linvels.append(vel)

        qvel = np.zeros(len(fpos) - 1)
        if i > 0:
            prev = frame_pos[i - 1]
            curr = frame_pos[i]
            dt = frames[i - 1][0]
            ...
            r_lvel = (rc - rp) / dt
            r_avel = quat_rot(prev[3:7], curr[3:7]) / dt

            hvel = (curr[7:] - prev[7:]) / dt

            qvel = np.concatenate([r_lvel, r_avel, hvel])
        angvels.append(qvel)


        coms.append(fpos[:3])

        ends.append(data.geom_xpos[[6, 9, 12, 15]])

        # if i >= 0:
        #     print("fp", fpos[24 + 7], data.qpos[24 + 7], data.qvel)
        #     print(model.nq, model.nv, model.nu, model.na, model.nbody)
        #     print(fpos)
