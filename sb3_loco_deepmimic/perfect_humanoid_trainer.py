...
class DeepMimicRewardWrapper(gym.Wrapper):
    def __init__(self, env, reference_data, state_mean, state_std, action_mean, action_std,
                 # CHANGED: Further increased imitation weight, reduced velocity weight and task weight.
    ...
    def reset(self, **kwargs):
        ...
        # Reset episode metrics and smoothness tracker
        self.running_reward = 0
        self.episode_len = 0
        self.prev_obs = None  # CHANGED: Reset previous observation
        # Use a random starting index to vary the phase of the reference motion.
    ...
    def step(self, action):
        ...
        # Add smoothness penalty (penalizes rapid changes in observation)
        ...
        # Add fall penalty if fallen and mark termination
...
class RenderAndRecordCallback(BaseCallback):
    """
    At every record_freq timesteps, runs a short rollout in a separate rendering
    environment and saves the run as a video.
    """
...
if __name__ == "__main__":
    ...
    # Function to create a rendering environment for the callback.
    def make_render_env():
        env = gym.make("LocoMujoco",
                       env_name="HumanoidTorque.run",
                       dataset_type="perfect",
